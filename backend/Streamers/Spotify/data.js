...
const limiter = new Bottleneck({
  minTime: 50 // Adjust this value as needed
});

// Utility function for making requests with retry logic
...
router.get('/tracks', (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        ...
        try {
            let track = await makeRequestWithRetry(() => instance.get(`/tracks/${id}`));
...
router.get('/new-releases', (req, res) => {
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data = await makeRequestWithRetry(() => instance.get(`/browse/new-releases?limit=50&offset=0`));
                
                let filteredAlbums = await Promise.all(data.data.albums.items.map(async (album) => {
                    let tracksData = await makeRequestWithRetry(() => instance.get(`/albums/${album.id}/tracks?market=ES`));
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let artistsData;
                if (ids) {
                    artistsData = await makeRequestWithRetry(() => instance.get(`/artists?ids=${ids}`));
                } else {
                    let newAlbums = await makeRequestWithRetry(() => instance.get(`/browse/new-releases?limit=15&offset=0`));
                    ...
                    artistsData = await makeRequestWithRetry(() => instance.get(`/artists?ids=${idArray.join(',')}`));
                }

                let filteredArtists = await Promise.all(artistsData.data.artists.map(async (artist) => {
                    let topTracksData = await makeRequestWithRetry(() => instance.get(`artists/${artist.id}/top-tracks?market=ES`));
...
router.get(`/artists/:id/albums`, (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data = await makeRequestWithRetry(() => instance.get(`artists/${id}/albums`));
...
router.get(`/artists/:id/top-tracks`, (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data = await makeRequestWithRetry(() => instance.get(`artists/${id}/top-tracks?market=ES`));
...
router.get(`/artists/:id/related-artists`, (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data = await makeRequestWithRetry(() => instance.get(`artists/${id}/related-artists`));
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data;
                if (ids) {
                    data = await makeRequestWithRetry(() => instance.get(`/albums?ids=${ids}&market=ES`));
                } else {
                    data = await makeRequestWithRetry(() => instance.get("albums?ids=7aJuG4TFXa2hmE4z1yxc3n,1NAmidJlEaVgA3MpcPFYGq,5EYKrEDnKhhcNxGedaRQeK,3RQQmkQEvNCY4prGKE6oc5,4iqbFIdGOTzXeDtt9owjQn,18NOKLkZETa4sWwLMIm0UZ,6s84u2TUpR3wdUv4NgKA2j,6i7mF7whyRJuLJ4ogbH2wh,07w0rG5TETcyihsEIZR3qG,5quMTd5zeI9yW5UDua8wS4,168CdR21lfn0TTyw1Pkdcm,78bpIziExqiI9qztvNFlQu,3zu0hJJew2qXZNlselIQk8,5tSQtQGkrCJx3hoQxmLgfM,7txGsnDSqVMoRl6RQ9XyZP&market=ES"));
                }

                // Filter albums with preview URLs
                let filteredAlbums = data.data.albums.filter(album => {
                    let tracksWithPreviews = album.tracks.items.filter(track => track.preview_url);
                    if (tracksWithPreviews.length > 0) {
                ...
                data.data.albums = filteredAlbums;
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data;
                if (id) {
                    data = await makeRequestWithRetry(() => instance.get(`/playlists/${id}`));
                    ...
                    data = await makeRequestWithRetry(() => instance.get('/browse/featured-playlists?offset=0&limit=20'));
                    data.data.playlists.items = await Promise.all(data.data.playlists.items.map(async (playlist) => {
                        let playlistData = await makeRequestWithRetry(() => instance.get(`/playlists/${playlist.id}`));
...
router.get(`/albums/:id/tracks`, (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                let data = await makeRequestWithRetry(() => instance.get(`albums/${id}/tracks?market=ES`));
                return res.status(200).json({ ...data?.data });
            } catch (error) {
                console.log(error);
...
router.get('/search', (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        if (instance) {
            try {
                const data = await makeRequestWithRetry(() => instance.get(`/search?q=${encodeURIComponent(q)}&type=${searchTypes}&limit=20`));
...
router.get('/fetchSong', (req, res) => {
    ...
    SpotifyConn(async (error, instance) => {
        ...
        try {
            let trackResponse = await makeRequestWithRetry(() => instance.get(`/tracks/${id}`));
            if (!trackResponse.data.preview_url) {
                return res.status(404).json({ error: 'No preview URL available for this track' });
            }
            let track = trackResponse.data;

            let audioFeaturesResponse = await makeRequestWithRetry(() => instance.get(`/audio-features/${id}`));
